 ...
 *      2，配置spring-redis配置主机地址和端口号
 *      3，使用StringRedisTemplate
 *
 *  7,整和redisson作为分布式锁等功能
 *      1,引入依赖
 *      2，配置
 *      3,使用：
 *      .
 *  8，整和springcache简化缓存开发
 ...
 *    自动配置了RedisCacheManager-》初始化所有缓存->每个缓存决定用什么配置
 *    ——》如果RedisCacheConfiguration有就用自己的，没有就用默认配置
 *    ——》想改缓存的配置自需要给容器放一个RedisCacheConfiguration即可
 *
 *
 *
 */
